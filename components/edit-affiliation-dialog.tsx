...
export function EditAffiliationDialog({ isOpen, onClose, affiliation }: EditAffiliationDialogProps) {
  ...
  useEffect(() => {
    if (affiliation) {
      setSelectedProduct(affiliation.productId);
      setCommissionPercent(affiliation.commission.toString());
      setExpiryDate(new Date(affiliation.expiryDate));
    }
  }, [affiliation]);
      ...
      <DialogContent className="bg-white p-0 gap-0 max-w-4xl">
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-4xl font-bold text-gray-900">Edit Affiliation</h2>
            <Button
              onClick={onClose}
              className="bg-green-400 hover:bg-green-500 text-white px-8 py-6 text-base font-medium rounded-lg"
            >
              Retour
            </Button>
          </div>
